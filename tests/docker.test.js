...
const _TEST_EXPECTED_MASTER_KEY =
...
function runDockerCommand(command, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Use project root directory for Docker commands
    const workingDir = options.cwd || path.join(__dirname, "..");
...
describeDocker("Docker Integration Tests", () => {
  ...
  describe("Docker Image Building", () => {
    test("should build Docker image successfully", async () => {
      ...
      try {
        const result = await runDockerCommand(buildCommand, {
          timeout: 300000,
        });
        expect(result).toBeDefined();
      } catch (error) {
        // Check if this is a known issue with Docker user creation in CI
        if (
          error.message.includes("groupadd") ||
          error.message.includes("useradd") ||
          error.message.includes("exit code: 1")
        ) {
          console.warn(
            "Docker build failed due to user creation issues in CI environment:",
            error.message,
          );
          // Skip test in CI environments where user creation fails
          if (process.env.CI || process.env.GITHUB_ACTIONS) {
            console.warn(
              "Skipping Docker build test in CI due to known user creation limitations",
            );
            return;
          }
        }
  ...
  describe("Key Generation Workflows", () => {
    test("should generate master key with test mnemonic", async () => {
      ...
      expect(result.stdout).toMatch(new RegExp(_TEST_EXPECTED_MASTER_KEY));
    });
    ...
    test("should handle output directory gracefully in NON_INTERACTIVE mode", async () => {
      ...
      const command = `docker run --rm ${getPlatformFlag()} -i -v "${TEST_OUTPUT_DIR}:/output" -e NON_INTERACTIVE=1 ${TEST_IMAGE_NAME} sh -c 'echo "${_TEST_EXPECTED_MASTER_KEY}" | ./convert.sh /output'`;
  ...
  describe("Error Handling", () => {
    test("should handle invalid master key gracefully", async () => {
      ...
      expect(result.stderr).toMatch(/Unrecognized input format/i);
    });

    test("should handle missing volume mount gracefully", async () => {
      const command = `docker run --rm ${getPlatformFlag()} -i -e NON_INTERACTIVE=1 ${TEST_IMAGE_NAME} sh -c 'echo "${_TEST_EXPECTED_MASTER_KEY}" | ./convert.sh /nonexistent'`;
...
describe("Shell Script Tests", () => {
  ...
  describe("convert.sh output directory handling", () => {
    ...
    test("should create new directory when it does not exist", () => {
      ...
      try {
        execSync(
          `echo "${_TEST_EXPECTED_MASTER_KEY}" | NON_INTERACTIVE=1 SKIP_ENCRYPTION=1 ./convert.sh "${TEST_OUTPUT_DIR}"`,
          {
            stdio: "ignore",
...
const describeFreshDockerTests = isDockerAvailable() ? describe : describe.skip;

describeFreshDockerTests("Fresh BIP39 Seed Docker Tests", () => {
  const bip39 = require("bip39");
  ...
  test("should generate fresh mnemonic and convert in Docker container", async () => {
    ...
    expect(masterKeyHex).not.toBe(_TEST_EXPECTED_MASTER_KEY);
