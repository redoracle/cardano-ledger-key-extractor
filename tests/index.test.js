...
describe("Fresh BIP39 Seed Generation Tests", () => {
  ...
  test("fresh seed integration with full workflow", () => {
    ...
    try {
      ...
      // Try to run convert.sh if available
      if (fs.existsSync("./convert.sh")) {
        try {
          // Create test output directory
          fs.mkdirSync(testOutputDir, { recursive: true });

          // Run conversion with fresh master key
          const convertCommand = `echo "${masterKeyHex}" | NON_INTERACTIVE=1 ../convert.sh ${testOutputDir}`;
        ...
        console.log("convert.sh not found, skipping integration test");
      }
    } finally {
      cleanup();
    }
